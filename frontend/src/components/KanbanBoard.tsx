...
export const KanbanBoard = ({
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <main className="relative flex min-h-screen flex-col gap-6 px-6 pb-10 pt-6">
        <header className="flex items-center justify-between rounded-2xl border border-[var(--stroke)] bg-white/80 px-6 py-4 shadow-[var(--shadow)] backdrop-blur">
          <div className="flex items-center gap-4">
            <div className="h-8 w-1 rounded-full bg-[var(--accent-yellow)]" />
            <div>
          ...
          <div className="flex items-center gap-5">
            <div className="hidden items-center gap-3 md:flex">
              {board.columns.map((column) => (
                <div
                  key={column.id}
                  className="flex items-center gap-1.5 rounded-full border border-[var(--stroke)] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.15em] text-[var(--navy-dark)]"
                >
        ...
        >
          <section className="grid flex-1 auto-rows-min grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {board.columns.map((column) => (
              <KanbanColumn
                key={column.id}
                column={column}
